<div class="bg-white p-4 rounded shadow-sm mb-3">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="index.php?page=all-best-selling-monitor" class="text-primary fst-italic text-decoration-none">
            Xem tất cả →
        </a>
    </div>
    ...
    <div class="product-scroll d-flex overflow-auto pb-2" style="gap: 15px;">
        <?php
        // Lấy dữ liệu từ mảng $data đã được controller nạp vào
        $bestSellingMonitors = $data['best_selling_monitors'] ?? [];
        ?>

        <?php if (!empty($bestSellingMonitors)): ?>
            <?php foreach ($bestSellingMonitors as $monitor): ?>
                <div class="product-item" style="min-width: 230px; max-width: 230px;">
                    <div class="card product-card border h-100 d-flex flex-column shadow-sm transition hover-up">
                        <a href="index.php?page=product-detail&slug=<?php echo $monitor['slug']; ?>"
                            ...
                            <div class="position-relative product-img-container overflow-hidden bg-white">
                                <img class="card-img-top p-2 product-img-hover"
                                    src="<?php echo $monitor['thumbnail'] ? 'admin/admin_images/' . $monitor['thumbnail'] : 'admin/admin_images/post_thumbnails/undefined.png'; ?>"
                                    style="height:180px; object-fit:contain;"
                                    alt="<?php echo htmlspecialchars($monitor['name']); ?>">

                                <?php if ($monitor['sale_price'] > 0): ?>
                                ...
                                <div class="mt-auto pt-2">
                                    <?php if ($monitor['sale_price'] > 0): ?>
                                        <div class="text-danger fw-bold fs-5">
                                            <?php echo number_format($monitor['sale_price'], 0, ',', '.'); ?> đ
                                        </div>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="text-muted small text-decoration-line-through">
                                                <?php echo number_format($monitor['price'], 0, ',', '.'); ?> đ
                                            </div>
                                            <div class="badge border border-danger text-danger fw-bold" style="font-size: 0.7rem;">
                                                -<?php echo round((($monitor['price'] - $monitor['sale_price']) / $monitor['price']) * 100); ?>%
                                            </div>
                                        </div>
                                    <?php else: ?>
                                        <div class="text-danger fw-bold fs-5">
                        ...
                        <div class="card-footer p-2 border-0 bg-transparent text-center mt-auto">
                            ...
                            <div class="d-flex align-items-center mt-2 mb-1 justify-content-between">
                                ...
                                <span class="text-muted small">
                                    ...
                                    <?php echo number_format($monitor['total_sold'] ?? 0); ?>
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p class="p-4 text-muted">Hiện chưa có màn hình bán chạy nào.</p>
        <?php endif; ?>
    </div>
</div>