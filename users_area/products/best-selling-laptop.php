<div class="bg-white p-4 rounded shadow-sm mb-3">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="index.php?page=all-best-selling-laptop" class="text-primary fst-italic text-decoration-none">
            Xem tất cả →
        </a>
    </div>
    ...
    <div class="product-scroll d-flex overflow-auto pb-2" style="gap: 15px;">
        <?php
        // Lấy dữ liệu từ biến $data đã được route index.php truyền vào
        $bestSellingLaptops = $data['best_selling_laptops'] ?? [];
        ?>

        <?php if (!empty($bestSellingLaptops)): ?>
            <?php foreach ($bestSellingLaptops as $laptop): ?>
                <div class="product-item" style="min-width: 230px; max-width: 230px;">
                    <div class="card product-card border h-100 d-flex flex-column shadow-sm transition hover-up">
                        <a href="index.php?page=product-detail&slug=<?php echo $laptop['slug']; ?>"
                            ...
                            <div class="position-relative product-img-container overflow-hidden bg-white">
                                <img class="card-img-top p-2 product-img-hover"
                                    src="<?php echo $laptop['thumbnail'] ? 'admin/admin_images/' . $laptop['thumbnail'] : 'admin/admin_images/post_thumbnails/undefined.png'; ?>"
                                    style="height:180px; object-fit:contain;"
                                    alt="<?php echo htmlspecialchars($laptop['name']); ?>">

                                <?php if ($laptop['sale_price'] > 0): ?>
                                ...
                                <div class="mt-auto pt-2">
                                    <?php if ($laptop['sale_price'] > 0): ?>
                                        <div class="text-danger fw-bold fs-5">
                                            <?php echo number_format($laptop['sale_price'], 0, ',', '.'); ?> đ
                                        </div>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="text-muted small text-decoration-line-through">
                                                <?php echo number_format($laptop['price'], 0, ',', '.'); ?> đ
                                            </div>
                                            <div class="badge border border-danger text-danger fw-bold" style="font-size: 0.7rem;">
                                                -<?php echo round((($laptop['price'] - $laptop['sale_price']) / $laptop['price']) * 100); ?>%
                                            </div>
                                        </div>
                                    <?php else: ?>
                                        <div class="text-danger fw-bold fs-5">
                        ...
                        <div class="card-footer p-2 border-0 bg-transparent text-center mt-auto">
                            ...
                            <div class="d-flex align-items-center mt-2 mb-1 justify-content-between">
                                ...
                                <span class="text-muted small">
                                    ...
                                    <?php echo number_format($laptop['total_sold'] ?? 0); ?>
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p class="p-4 text-muted">Đang cập nhật danh sách laptop bán chạy...</p>
        <?php endif; ?>
    </div>
</div>